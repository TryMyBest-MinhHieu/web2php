...
if (isset($_POST['submit'])) {
   ...
   //    } else {
         $update_name = $conn->prepare("UPDATE `admin` SET name = ?, role_id = ? WHERE id = ?");
         $update_name->execute([$name, $role, $update_id]);
         $message[] = 'Succesfully!';
   //    }
   // }
   ...
   $select_old_pass->execute([$update_id]);
   ...
   if ($old_pass != $empty_pass) {
      if ($old_pass != $prev_pass) {
         ...
         if ($new_pass != $empty_pass) {
            $update_pass = $conn->prepare("UPDATE `admin` SET password = ?, role_id = ?  WHERE id = ?");
            $update_pass->execute([$confirm_pass, $role, $update_id]);
...
<body>
   ...
   <section class="form-container">
      ...
      <form action="" method="POST">
         ...
         <input type="text" name="name" maxlength="20" class="box" oninput="this.value = this.value.replace(/\s/g, '')" placeholder="Tên" value="<?= isset($_GET['name']) ? $_GET['name'] : $fetch_profile['name']; ?>">
         ...
         <select id="roles" name="roles" class="box">
            ...
            if ($select_roles->rowCount() > 0) {
               while ($fetch_role = $select_roles->fetch(PDO::FETCH_ASSOC)) {
                  if((int)$_GET['role']==$fetch_role['role_id']){
                     ?>
                     <option selected value=<?= $fetch_role["role_id"]; ?>><?= $fetch_role["role_name"]; ?></option>'
                     <?php
                  }
                  else{
                     ?>
